...
- Name: tier2_boundary_edge.spec.ts >> Tier 2: Password Recovery Boundary (Conditional) >> 30. Password Recovery: Submit reset password with invalid token (skipped if missing)
- Location: tests\tier2_boundary_edge.spec.ts:381:7

# Error details

```
Error: page.goto: net::ERR_CONNECTION_REFUSED at http://localhost:3000/login/reset-password#error=unauthorized
Call log:
  - navigating to "http://localhost:3000/login/reset-password#error=unauthorized", waiting until "load"
