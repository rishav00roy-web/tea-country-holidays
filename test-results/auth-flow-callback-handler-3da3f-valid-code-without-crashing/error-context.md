...
- Name: auth-flow.spec.ts >> callback handler redirects to homepage on invalid code without crashing
- Location: tests\auth-flow.spec.ts:3:5

# Error details

```
Error: page.goto: net::ERR_CONNECTION_REFUSED at http://localhost:3000/auth/callback?code=invalid-test-code
Call log:
  - navigating to "http://localhost:3000/auth/callback?code=invalid-test-code", waiting until "load"
