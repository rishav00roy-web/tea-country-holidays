    ...
    <section id="who-we-are" className="py-20 md:py-32 bg-brand-floral overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          <div ref={leftRef}>
            <span
              className="text-brand-red font-bold text-sm tracking-[0.2em] uppercase block mb-4 opacity-0"
              style={{ animation: leftVisible ? "fadeSlideRight 0.7s ease forwards 0s" : "none" }}
            >
              Our Story
            </span>
            <h2
              className="font-serif text-4xl md:text-5xl lg:text-6xl font-bold text-brand-evergreen dark:text-white leading-[1.1] mb-6 opacity-0"
              style={{ animation: leftVisible ? "fadeSlideRight 0.7s ease forwards 0.12s" : "none" }}
            >
              Who We <span className="text-brand-gold italic">Are</span>
            </h2>
            <p
              className="text-brand-ink/65 text-lg leading-relaxed mb-5 opacity-0"
              style={{ animation: leftVisible ? "fadeSlideRight 0.7s ease forwards 0.22s" : "none" }}
            >
            ...
            <p
              className="text-brand-ink/65 text-lg leading-relaxed mb-8 opacity-0"
              style={{ animation: leftVisible ? "fadeSlideRight 0.7s ease forwards 0.32s" : "none" }}
            >
            ...
            <div
              ...
              style={{ animation: leftVisible ? "fadeSlideRight 0.7s ease forwards 0.4s" : "none" }}
            >
            ...
            <div
              ...
              style={{ animation: leftVisible ? "fadeSlideRight 0.7s ease forwards 0.5s" : "none" }}
            >
              {STATS.map(([n, l]) => (
          ...
          <div ref={rightRef} className="grid sm:grid-cols-2 gap-5">
            {pillars.map((p, i) => {
              ...
              return (
                <div
                  key={p.title}
                  className="group bg-white dark:bg-[#12291f] border border-brand-gold/10 dark:border-brand-gold/25 rounded-2xl p-6 hover:border-brand-gold/50 hover:shadow-xl hover:-translate-y-1.5 transition-all duration-400 cursor-default opacity-0"
                  style={{
                    animation: rightVisible
                      ? `fadeSlideUp 0.65s ease forwards ${0.05 + i * 0.1}s`
                      : "none",
                  }}
                >
