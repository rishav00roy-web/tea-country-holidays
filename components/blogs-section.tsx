        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {blogs.map((blog) => (
            <Link
              key={blog.id}
              href={`/blog/${blog.slug}`}
              className="group bg-white/80 dark:bg-brand-ink/40 backdrop-blur-md rounded-3xl overflow-hidden shadow-[0_4px_24px_rgba(0,0,0,0.04)] hover:shadow-[0_12px_40px_rgba(212,175,55,0.12)] border border-brand-evergreen/5 dark:border-brand-sand/10 transition-all duration-500 hover:-translate-y-1.5 flex flex-col h-full relative"
            >
              {/* Image */}
